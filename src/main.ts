...
window.onload = () => {
    ...
    submitButton.addEventListener('click', function handleClick(event) {
        const input = inputArea.value.toLowerCase();

        // find the index of user input in the read array
        const optionIndex = storyOutput[storyOutputIndex].read.indexOf(input);
        ...
        if (optionIndex > -1) {
            ...
            const storyOutputName = storyOutput[storyOutputIndex].id[optionIndex];



            // find the index of the option in the storyOutput array corresponding to the story output id selected by the user
            storyOutputIndex = storyOutput.findIndex(s => s.name === storyOutputName);
            ...
            while (storyOutput[storyOutputIndex].lead) {
                storyOutputIndex = storyOutput.findIndex(s => s.name === storyOutput[storyOutputIndex].lead);
...
function buildOutput(value: string, addQuestionMark: boolean = true): string {

    let output = value;

    if (storyOutput[storyOutputIndex].lore && storyOutput[storyOutputIndex].lore.length > 0) {
        for (let lore of storyOutput[storyOutputIndex].lore) {
            output += '\n' + lore;
        }
    }
